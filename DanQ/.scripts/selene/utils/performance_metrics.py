...
def visualize_roc_curves(
    prediction, target, output_dir, report_gt_feature_n_positives=50,
    style="seaborn-colorblind", fig_title="Feature ROC curves",
    dpi=500
):
    """
...
def visualize_precision_recall_curves(
    prediction, target, output_dir, report_gt_feature_n_positives=50,
    style="seaborn-colorblind", fig_title="Feature precision-recall curves",
    dpi=500
):
    """
...
def compute_score(
    prediction, target, metric_fn, report_gt_feature_n_positives=10
):
    """
    Using a user-specified metric, computes the distance between two tensors.
    ...
    average_score, feature_scores : tuple(float, numpy.ndarray)
        ...
        `(None, [])`.
    """
    for z in zip(prediction, target):
        print(z)
    exit(0)
...
def get_feature_specific_scores(data, feature_index):
    """
    ...
    feature_index : dict
        A dictionary mapping feature indices (`int`) to names (`str`).
    Returns
    -------
    dict
...
class PerformanceMetrics(object):
    ...
    def remove_metric(self, name):
        """
        Ends the tracking of the specified metric, and returns the previous
        scores associated with that metric.
    ...
    def update(self, prediction, target):
        """
        Evaluates the tracked metrics on a model prediction and its target
        value, and adds this to the metric histories.
